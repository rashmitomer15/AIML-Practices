...
#unknown_image1 = face_recognition.load_image_file("group1.jpg")
unknown_image1 = face_recognition.load_image_file("group2.jpg")
...
un_face_locations = face_recognition.face_locations(rgb_small_frame)
unknown_face_encodings = face_recognition.face_encodings(rgb_small_frame, un_face_locations)
...
for (top, right, bottom, left), name in zip(un_face_locations, face_names):
   ...
   # cv2.rectangle(unknown_image1, (left, bottom - 35), (right, bottom), (0, 255, 0), cv2.FILLED)
    ...
    #cv2.putText(unknown_image1, name, (left + 6, bottom - 6), font,  2.0, (255, 255, 255), 1)
    y = top - 15 if top - 15 > 15 else top + 15
    cv2.putText(unknown_image1, name, (left, y), font, 4.0, (0, 255, 0), 2)

# Display the resulting image
small_frame_new = cv2.resize(unknown_image1, (0, 0), fx=0.15, fy=0.15)
rgb_small_frame_new = small_frame_new[:, :, ::-1]
cv2.imshow('Result', rgb_small_frame_new)
cv2.waitKey(0)
